...
from .reporting import calibration_markdown_report, calibration_rows_csv, calibration_summary_csv
...
def main() -> None:
    ...
    parser.add_argument("--rows-out", type=str, default="")
    parser.add_argument("--summary-out", type=str, default="")
    parser.add_argument("--report-md-out", type=str, default="")
    ...
    print(
        "rank,config_id,score,avg_adjustment_objective,avg_critical_transition,avg_coupling_penalty,avg_applied_edits,avg_converged,avg_supervisory_confusion,avg_supervisory_forgetting,avg_longrun_churn,avg_longrun_retention,avg_longrun_diversity"
    )
    ...
    if args.summary_out:
        with open(args.summary_out, "w", encoding="utf-8") as f:
            f.write(calibration_summary_csv(summary))
    if args.rows_out:
        with open(args.rows_out, "w", encoding="utf-8") as f:
            f.write(calibration_rows_csv(rows))
    if args.report_md_out:
        with open(args.report_md_out, "w", encoding="utf-8") as f:
            f.write(calibration_markdown_report(rows, summary))
